...
=item htmlddlb (?field, name, data, field name,...) -> HTML drop-down list box

=item htmlddlb (?field, name, data, ?container, ?feed sub, field name,...)

Generate input helper drop-down list box HTML.
May be used in C<-inphtml>.
Optional 'field' parameter used for javascript is recognized as HTML or '$_' placeholder.
